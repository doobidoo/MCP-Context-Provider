...
def check_installation_files():
    ...
    if not server_script.exists():
        ...
        if server_script.exists():
            required_items[1] = (
                "server/context_provider_server.py",
                "file",
                "Main server script",
            )
...
def check_server_loading(install_dir):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "context_provider_server", server_script
        )
...
def check_claude_configuration():
    ...
    if not config_path:
        ...
        if platform.system() == "Windows":
            print_info(
                f"  - {Path(os.environ['APPDATA']) / 'Claude' / 'claude_desktop_config.json'}"
            )
        elif platform.system() == "Darwin":  # macOS
            print_info(
                f"  - {Path.home() / 'Library' / 'Application Support' / 'Claude' / 'claude_desktop_config.json'}"
            )
        else:  # Linux and others
            print_info(
                f"  - {Path.home() / '.config' / 'claude' / 'claude_desktop_config.json'}"
            )
        return False
...
def run_comprehensive_check():
    ...
    checks = [
        ...
        (
            "Context Files",
            lambda: (
                check_context_files(find_installation_directory())
                if find_installation_directory()
                else False
            ),
        ),
        (
            "Server Loading",
            lambda: (
                check_server_loading(find_installation_directory())
                if find_installation_directory()
                else False
            ),
        ),
