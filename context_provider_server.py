...
class MemoryServiceIntegration:
    ...
    async def store_memory(
        self, content: str, tags: List[str] = None, metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Storing in memory: {content[:100]}..."
    ...
    async def recall_memory(
        self, query: str, n_results: int = 5, tags: List[str] = None
    ) -> Dict[str, Any]:
...
class ContextProvider:
    ...
    def load_all_contexts(self):
        ...
        for file_path in context_files:
            try:
                ...
                print(
                    f"Error loading context file {file_path.name}: {e}", file=sys.stderr
                )
            ...
            if "pattern" in correction_rule and "replacement" in correction_rule:
                try:
                    ...
                    corrected_text = re.sub(
                        pattern, replacement, corrected_text, flags=re.MULTILINE
                    )
                except re.error as e:
                    print(
                        f"Error in regex pattern {correction_name}: {e}",
                        file=sys.stderr,
                    )
                    continue
    ...
    async def _store_context_change_in_memory(
        self, operation: str, context_name: str, details: Dict[str, Any]
    ):
        ...
        try:
            ...
            if operation == "created":
                ...
                content += (
                    f" - Updated fields: {', '.join(details.get('updated_fields', []))}"
                )
            ...
            if result["success"]:
                print(
                    f"Stored context change in memory: {operation} for {context_name}",
                    file=sys.stderr,
                )
            else:
                print(
                    f"Failed to store context change: {result.get('error')}",
                    file=sys.stderr,
                )
    ...
    try:
        if name == "get_tool_context":
            ...
            if not tool_name or not text:
                return [
                    TextContent(
                        type="text", text="Error: tool_name and text are required"
                    )
                ]
            ...
            if not name_arg or not category:
                return [
                    TextContent(
                        type="text", text="Error: name and category are required"
                    )
                ]
            ...
            if not context_name:
                return [
                    TextContent(type="text", text="Error: context_name is required")
                ]
            ...
            if not all([context_name, pattern_section, pattern_name]):
                return [
                    TextContent(
                        type="text",
                        text="Error: context_name, pattern_section, and pattern_name are required",
                    )
                ]

            result = provider.add_context_pattern(
                context_name, pattern_section, pattern_name, pattern_config
            )
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
            ...
            analysis = await provider.learning_engine.analyze_context_effectiveness(
                context_name
            )
            ...
            suggestions = await provider.learning_engine.proactive_context_suggestions(
                current_contexts
            )
            return [TextContent(type="text", text=json.dumps(suggestions, indent=2))]
            ...
            result = await provider.auto_optimize_context(
                context_name, apply_optimizations
            )
            return [TextContent(type="text", text=json.dumps(result, indent=2))]
